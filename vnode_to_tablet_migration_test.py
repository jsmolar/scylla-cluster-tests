...
class VnodeToTabletMigrationTest(LongevityTest):
    ...
    def restart_node_after_migration(self, node) -> None:
        ...
        # Suppress ClusterHealthValidatorEvent.NodeStatus for this node while it is down for
        # resharding.  A large reshard (hundreds of GB) can take 15+ minutes and the health
        # validator would otherwise fire a CRITICAL event for the node being DN, causing a
        # false-positive test failure.  The test itself controls the restart and waits for the
        # node to come back up, so the health alert carries no additional diagnostic value here.
        with EventsFilter(
